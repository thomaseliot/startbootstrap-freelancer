...
uint8_t left_text_mode = 0;   // is the display showing text (1) or numerical value (0) ?
uint8_t right_text_mode = 0;
short left_value = 0;         // numerical value currently displayed on display
short right_value = 0;
char left_string[3] = "   ";  // string currently displayed on the display
char right_string[3] = "   ";
char left_rgb = 0b000;		  // current color of the display
char right_rgb = 0b000;
char left_LEDs = 0b0000;      // current LED pattern on the display
char right_LEDs = 0b0000;
...
void display_int(char which_display, short val, char rgb, char LEDs) {
	switch(which_display) {
		case 1:
			if (left_text_mode == 0 && left_value == val && left_rgb == rgb && left_LEDs == LEDs) {
				return;
			}
		...
		case 2:
			...
			break;
		case 3:
			if (right_text_mode == 0 && right_value == val && right_rgb == rgb && right_LEDs == LEDs) {
				return;
			}
