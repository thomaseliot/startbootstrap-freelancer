	    ...
	    void	*closure)
{
	...
	{
	    fbBltOne ((FbStip *) (src + (INT32)(pbox->y1 + dy + srcYoff) * (INT32)srcStride),
	    ...
	    void	*closure)
{
	...
	if (pDstDrawable->bitsPerPixel == 1)
	{
	    ...
	    fbBltPlane (src + (INT32)(pbox->y1+ dy + srcYoff) * (INT32)srcStride,
			srcStride,
			(pbox->x1 + dx + srcXoff) * srcBpp,
			srcBpp,
	...
	else
	{
	    ...
	    tmp = xalloc ((INT32)tmpStride * (INT32)height * sizeof (FbStip));
	    if (!tmp)
		return;
