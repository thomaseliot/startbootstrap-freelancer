...
fbValidateGC(GCPtr pGC, unsigned long changes, DrawablePtr pDrawable)
{
    ...
    */
    if (changes & (GCForeground|GCBackground|GCPlaneMask|GCFunction))
    {
...
#ifdef FB_TRANS
        unsigned char saveALU = pGC->alu;

        /* Transparent drawables require alpha channel to be set */
        if (pGC->transDraw)
        {
            switch (pDrawable->bitsPerPixel)
            {
            case 32:
                transMask = ALPHA_MASK_32;
                break;
            case 16:
                transMask = ALPHA_MASK_16;
                break;
            case 8:
                /* For 8-bit, the color is the alpha.
                ** Do not set the transMask nor override the alu!
                */
            default:
                break;
            }
        }
        else if (pGC->alphaBuffer)
        {
            /* Drawing to an alpha buffer without transDraw which means
            ** we're doing a clear operation.
            */
            if (8 == pDrawable->bitsPerPixel)
            {
                /* This is a non-trans draw into an alpha buffer.
                ** For this case we need to clear the alpha.
                */
                pGC->alu = GXclear;
            }
        }
        ...
        pPriv->fg = pGC->fgPixel & mask | transMask;
        pPriv->bg = pGC->bgPixel & mask | transMask;
        ...
        if (s == 0)
            _altiaErrorMessage(ALT_TEXT("No bits per pixel in fb"));
        else
        {
            while (s < FB_UNIT)
            {
