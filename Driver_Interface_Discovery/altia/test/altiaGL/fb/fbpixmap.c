...
fbCreatePixmapBpp (ScreenPtr pScreen, int width, int height, int depth, int bpp)
{
    ...
    paddedWidth = (((INT32)width * (INT32)bpp + FB_MASK) >> FB_SHIFT) * sizeof (FbBits);
    datasize = (INT32)height * paddedWidth;
#ifdef PIXPRIV
    ...
    pPixmap->drawable.alphas = (void *)0;
#ifdef FB_DEBUG
...
fbValidateDrawable (DrawablePtr pDrawable)
{
    ...
    first = bits - stride;
    last = bits + (INT32)stride * (INT32)pDrawable->height;
